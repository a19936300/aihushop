...
logback原生实现sl4j日志接口，加上log4j最近版本严重bug，改用logback也不错。
spring-boot-starter-logging自带实现了local-back,所以只要在启动类下面的resource中添加logback-spring.xml就行。
...
<configuration scan="true" scanPeriod="60 seconds" debug="false">
    ...
    <!--用于区分不同应用程序的记录-->
    <contextName>aihushop-api-logback</contextName>
